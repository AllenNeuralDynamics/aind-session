...
def sort_data_assets(assets: Iterable[codeocean.data_asset.DataAsset]) -> tuple[codeocean.data_asset.DataAsset, ...]:
    """Sort data assets by ascending creation date"""
    return tuple(sorted(assets, key=lambda asset: asset.created))

def get_data_asset(asset_id: str | uuid.UUID | codeocean.data_asset.DataAsset) -> codeocean.data_asset.DataAsset:
...
def is_raw_data_asset(asset: str | uuid.UUID | codeocean.data_asset.DataAsset) -> bool:
    ...
    if asset.custom_metadata and asset.custom_metadata.get("data level") == "raw data":
        logger.debug(f"{asset.id=} determined to be raw data based on custom_metadata containing 'data level': 'raw data'")
        return True
    else:
        logger.debug(f"{asset.id=} has no custom metadata")
    if asset.tags and any("raw" in tag for tag in asset.tags):
        logger.debug(f"{asset.id=} determined to be raw data based on tag(s) containing 'raw'")
        return True
    else:
        logger.debug(f"{asset.id=} has no tags")
    logger.info(f"No custom metadata or tags for {asset.id=}: determining if raw data asset based on name alone")
    try:
        session_id = str(npc_session.AINDSessionRecord(asset.name))
    except ValueError:
        logger.debug(f"{asset.id=} name does not contain a valid session ID: {asset.name=}")
        return False
    if session_id == asset.name:
        logger.debug(f"{asset.id=} name is a session ID alone, with no additional suffixes: it is considered raw data {asset.name=}")
        return True
    else:
        logger.debug(f"{asset.id=} name is not a session ID alone: it is not considered raw data {asset.name=}")
        return False

def get_data_asset_source_folder(asset: str | uuid.UUID | codeocean.data_asset.DataAsset) -> upath.UPath:
    ...
    """
    def get_folder_from_known_s3_locations(asset: codeocean.data_asset.DataAsset) -> upath.UPath:
        for key in (asset.id, asset.name):
            with contextlib.suppress(FileNotFoundError):
                return aind_session.utils.get_source_folder_by_name(key, ttl_hash=aind_session.utils.get_ttl_hash(10 * 60))
        raise FileNotFoundError(f"No source folder found for {asset.id=} or {asset.name=} in known S3 buckets")

    asset = get_data_asset(asset)
    if asset.source_bucket:
        protocol = {'aws': 's3', 'gcp': 'gs', 'local': 'file'}.get(asset.source_bucket.origin)
        if protocol:
            path =  upath.UPath(
                f"{protocol}://{asset.source_bucket.bucket}/{asset.source_bucket.prefix}"
            )
            if not path.exists():
                raise FileNotFoundError(f"{path.as_posix()} found from data asset, but does not exist (or access is denied)")
            logger.debug(f"Path for {asset.name}, {asset.id} returned (existence has been checked): {path.as_posix()}")
            return path
        else:
            logger.warning(f"Unsupported storage protocol: {asset.source_bucket.origin} for {asset.id}, {asset.name}")
    else:
        logger.debug(f"No source_bucket metadata available for {asset.id}, {asset.name}")
    ...
    **search_parameters,
    ...
    for key in ('limit', 'offset'):
        if key in search_parameters:
            logger.warning(f"Removing {key} from user-provided search_parameters: pagination is handled by this function")
            search_parameters.pop(key)
    if 'query' in search_parameters:
        logger.warning("Removing query from user-provided search_parameters: subject ID is used to assets")
    ...
    try:
        ...
        logger.warning(f"Subject ID {subject_id=} does not appear to be a Labtracks MID: assets may not be found")
    ...
    while offset < max_pages:
        ...
        raise TimeoutError(f"Max pages reached fetching codeocean data assets for {subject_id=}: {max_pages=}")
...
def get_session_data_assets(
    ...
    **search_parameters,
    ...
    if subject_id is None:
        raise ValueError(f"Search term must include a subject ID: {session_id_or_search_term=!r}")
    subject_assets = get_subject_data_assets(subject_id, ttl_hash=aind_session.utils.get_ttl_hash(), **search_parameters)
    return sort_data_assets(
        asset
        for asset in subject_assets
        if session_id_or_search_term in asset.name
        )
