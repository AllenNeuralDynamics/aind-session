...
class Metadata(aind_session.extension.ExtensionBaseClass):
    ...
    def __getattr__(self, name: str) -> str:
        ...
        try:
            ...
            raise AttributeError(f"No raw data folder found for {self._session.id}") from None
        try:
            content = (path := folder / f"{name}.json").read_text()
        except FileNotFoundError:
            raise AttributeError(f"No {name!r} metadata json found in {folder}") from None
        else:
    ...
    def get_available_json_paths(self) -> tuple[upath.UPath, ...]:
        ...
        """
        return tuple(sorted((path for path in self._session.raw_data_folder.iterdir() if path.suffix == ".json"), key=lambda p: p.name))
