...
class Ecephys(aind_session.extension.ExtensionBaseClass):
    ...
    def sorted_data_asset(self) -> codeocean.data_asset.DataAsset:
        ...
        """
        assets = tuple(asset for asset in self._session.assets if self.is_sorted_data_asset(asset))
        if len(assets) == 1:
            asset = assets[0]
        elif len(assets) > 1:
            logger.warning(f"Found multiple raw data assets for {self._session.id}: latest asset will be used as raw data")
            asset = assets[-1]
        else:
            raise LookupError(f"No raw data asset found for {self._session.id}. Has session data been uploaded?")
    ...
    def sorted_data_folder(self) -> upath.UPath:
        ...
        try:
            ...
            raise FileNotFoundError(f"No sorted data asset found in CodeOcean for {self._session.id}. Has the session been sorted?") from None
        else:
            logger.debug(f"Using asset {self.sorted_data_asset.id} to find sorted data path for {self._session.id}")
            sorted_data_folder = aind_session.utils.codeocean_utils.get_data_asset_source_folder(self.sorted_data_asset)
            logger.debug(f"Sorted data path found for {self._session.id}: {sorted_data_folder}")
    ...
    def is_sorted_data_asset(asset_id: str | codeocean.data_asset.DataAsset) -> bool:
        ...
        try:
            ...
            logger.debug(f"{asset.name=} does not contain a valid session ID: determined to be not a sorted data asset")
            return False
        if asset.name.startswith(f"{session_id}_sorted"):
            logger.debug(f"{asset.name=} determined to be a sorted data asset based on name starting with '<session-id>_sorted'")
            return True
        else:
            logger.debug(f"{asset.name=} determined to be not a sorted data asset based on name starting with '<session-id>_sorted'")
            return False
