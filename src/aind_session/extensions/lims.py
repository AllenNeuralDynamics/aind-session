...
class LimsExtension(aind_session.extension.ExtensionBaseClass):
    ...
    @staticmethod
    def get_session(labtracks_mouse_id: str | int, lims_session_id: str | int) -> aind_session.Session:
        ...
        if not subject_sessions:
            raise ValueError(f'No sessions found for subject {labtracks_mouse_id} in CodeOcean - has raw data been uploaded?')
        for session in subject_sessions:
            logger.debug(f"Checking raw data folder: {session.raw_data_dir.as_posix()}")
            for modality_folder in (p for p in session.raw_data_dir.iterdir() if p.is_dir()):
                try:
                    matching_path = next(modality_folder.glob(f'*{lims_session_id}*'))
                except StopIteration:
                    continue
                else:
                    logger.info(f"Found path name containing lims session ID: {matching_path.as_posix()}")
                    return session
            else:
                logger.debug(f"No path names contain {lims_session_id} in {session.id} raw data folder")
        else:
            raise ValueError(f'No raw data paths containing {lims_session_id} found in raw data dirs in S3 for {labtracks_mouse_id} - has raw data been uploaded?')
    ...
    def extract_id(value: str) -> int:
        ...
        for component in components:
            ...
            if len(component) < 9:
                logger.debug(f'{len(component)=} is too short to be a lims ID: ignoring')
                continue
            try:
    ...
    def get_id(aind_session_id: str) -> int:
        ...
        for modality_folder in (p for p in session.raw_data_dir.iterdir() if p.is_dir()):
            for path in modality_folder.iterdir():
                try:
                    lims_id = LimsExtension.extract_id(path.name) # name instead of stem as path may be a dir name containung "."
                except ValueError:
                    continue
                else:
            ...
            raise ValueError(f"No path names contain a lims ID for paths in raw data folder for {aind_session_id!r}")
