...
class Session:
    ...
    def raw_data_asset(self) -> codeocean.data_asset.DataAsset:
        ...
        """
        assets = tuple(asset for asset in self.assets if aind_session.utils.is_raw_data_asset(asset))
        if len(assets) == 1:
            asset = assets[0]
        elif len(assets) > 1:
            logger.warning(f"Found multiple raw data assets for {self.id}: latest asset will be used as raw data")
            asset = assets[-1]
        else:
            raise LookupError(f"No raw data asset found for {self.id}. Has session data been uploaded?")
    ...
    def raw_data_folder(self) -> upath.UPath:
        ...
        try:
            ...
            with contextlib.suppress(FileNotFoundError):
                ...
                logger.debug(f"No raw data asset uploaded for {self.id}, but data folder found: {path}")
                return path
            raise FileNotFoundError(f"No raw data asset in CodeOcean and no folder in known data buckets on S3 for {self.id}") from None
        else:
            logger.debug(f"Using asset {self.raw_data_asset.id} to find raw data path for {self.id}")
            raw_data_folder = aind_session.utils.get_data_asset_source_folder(self.raw_data_asset)
